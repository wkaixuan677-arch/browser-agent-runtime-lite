...
- **阶段超时与异常收口**：打开页面、观察、规划、动作决策和工具执行共享任务截止时间；超时或异常也会生成终止轨迹。
- **可观测轨迹（Trajectory）**：保存经过脱敏、顺序明确的规划、动作、验证与恢复事件，并限制页面观察文本长度。
...
- `AgentPolicy`：动作决策策略；
- `BrowserToolAdapter`：可被统一超时守卫约束的浏览器工具适配器；
...
测试覆盖：证据门控完成、首屏已满足目标、语义目标恢复、假完成拒绝、动作后阻断、Memory 注入、跨域请求预拦截、阶段超时、Policy/工具异常、敏感信息脱敏和预算耗尽。GitHub Actions 会自动执行类型检查、全部测试和本地浏览器演示。

## 安全边界

- 只允许使用任务契约明确列出的来源作为完成证据；Playwright 可路由的跨域导航会在请求前拦截，Runtime 还会对每次观察再次校验来源；
- 内置演示与测试为每次执行创建独立的临时浏览器上下文；接入自定义页面时也应遵循同样隔离方式；
- 测试页面不接收账号或个人数据；
- 不绕过验证码、登录墙或访问控制；
- 步数、恢复次数和浏览器动作均有上限；统一 Promise 截止时间覆盖页面打开、观察、Policy 与工具阶段；
- 超时会调用浏览器适配器的取消入口、把当前 Runtime 标记为不可复用，并返回可审计的 `blocked` 结果；无法取消的第三方 Policy Promise 不会再被当前 Runtime 消费；
- 输出轨迹会移除本地绝对路径、URL 查询参数、输入文本、常见 Token/API Key、Bearer 凭据和邮箱，并将页面观察文本限制为 2,000 字符。

跨域守卫不是进程级或网络级沙箱。安全场景应使用独立 BrowserContext，并设置 `serviceWorkers: 'block'`，避免请求由 Service Worker 绕过 Playwright 路由。
...
- 内置浏览器适配器支持超时取消；任意第三方 Policy / Tool 仍应自行接入 `AbortSignal`，Runtime 不能撤销已经在外部系统产生的副作用；
- 本仓库不宣称已经验证真实网页泛化、多模型泛化或公司内部落地。

## 后续计划

- 增加模型无关的 LLM Adapter 和结构化输出约束；
- 为第三方 Policy / Tool 增加统一 `AbortSignal`，并补充截图证据与更严格的动作结果验证；
- 输出 JSONL 轨迹和轻量评测报告；
- 增加无需登录、可重复运行的真实网页任务。
